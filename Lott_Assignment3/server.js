...
app.get("/get_favorites", function (request, response) {
...
app.get("/add_to_cart", function (request, response) {
    //defines verification booleans that will be changed depending if errors are detected
    let valid = true;  
    let valid_num = true;
    let zero_qty = false;
    let qty_name = 'quantity'; //name of textbox in products_data.html
    let product = request.query['products_key']; //name of productCatagory
    let favorite_name = request.query["favorite-"]; //name of checkbox in products_data.html
...
app.post("/update_cart", function (request, response) {
    if (request.cookies.loggedIn == "true") {
        ...
        response.redirect('./cart.html?updated=Your Cart Has Been Updated!') // if user changes anything on the cart page send this to let them know the cart has been updated
    } else {
        response.redirect("/get_to_login?error=Not Logged In") // They need to be logged in if they want to check out, the button will change once logged in 
    }
});
...
app.post("/get_to_login", function (request, response) {
    ...
    var user_pass = generateCipher(POST['password']); // IR1 A2: Erin Tachino
    ...
    if (user_data[entered_email] != undefined) {
        if (user_data[entered_email].password == user_pass) {
            let loggedIn = true;
            if (fs.existsSync(fname)) {
                request.session.loginDate = new Date(); // IR4 A2 Daniel Lott - last time user logged in 
                user_data[entered_email].num_loggedIn += 1; // IR4 A2 Daniel Lott - counts how many times user logged in 
                
                
                //sends cookie back to the client, email, login status, and cart information
...
<th class="text-right" style="width:15%">Extended Price</th>`;
    ...
    transporter.sendMail(mailOptions, function (error, info) {
        ...
        request.session.destroy(); 
        // distorys the session for the user who is signed in
    });
});
